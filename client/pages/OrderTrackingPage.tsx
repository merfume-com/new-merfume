...
export default function OrderTrackingPage() {
  ...
  // Base URL for API - yahaan aapka actual base URL daalein
  const API_BASE_URL = "http://merfume.dpdns.org";
  ...
  const trackOrderByNumber = async () => {
    ...
    setError("");
    setTrackedOrder(null);
    setShiprocketTrackingLink(""); // Reset tracking link
    ...
    try {
      ...
      // Generate ShipRocket tracking link if shipment ID exists
      if (response.data.shipmentId || response.data.airwayBillNumber) {
        const trackingId = response.data.shipmentId || response.data.airwayBillNumber;
        setShiprocketTrackingLink(`https://shiprocket.co/tracking/${trackingId}`);
      } else if (response.data.orderId) {
        // Alternative: use order ID for tracking
        setShiprocketTrackingLink(`https://shiprocket.co/tracking?order_id=${response.data.orderId}`);
      } else {
        // Generic ShipRocket tracking page
        setShiprocketTrackingLink("https://shiprocket.co/tracking");
      }
    } catch (err: any) {
      console.error("Error tracking order:", err);
      if (err.response?.status === 404) {
          ...
          <div className="space-y-8">
            ...
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl border border-green-200 p-6 shadow-lg">
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  {/* ShipRocket Tracking Button */}
                  {shiprocketTrackingLink && (
                    <a 
                      href={shiprocketTrackingLink} 
                      target="_blank" 
            ...
            <Tabs defaultValue="details" value={activeStatusTab} onValueChange={setActiveStatusTab}>
              ...
              <TabsContent value="details" className="space-y-6">
                ...
                {/* ShipRocket Tracking Link (Additional Location) */}
                {shiprocketTrackingLink && (
                  <Card>
                    <CardHeader>
                      <CardTitle className="flex items-center gap-2">
                      ...
                      <CardDescription>
                        Track your shipment in real-time with ShipRocket
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                        <div className="flex-1">
                          <div className="font-medium text-blue-700 mb-1">ShipRocket Tracking Available</div>
                          <div className="text-sm text-blue-600">
                            Click the button to track your shipment with detailed updates
                          </div>
                        </div>
                        <a 
                          href={shiprocketTrackingLink} 
                        ...
                        <div className="mt-6 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200">
                          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <h4 className="font-semibold text-blue-800">Live Shipment Tracking</h4>
                                <p className="text-sm text-blue-600">
                                  Track your shipment with detailed updates on ShipRocket
                                </p>
                              </div>
                            </div>
                            <a 
