...
export default function FragranceCareTips() {
  ...
  // Smooth scroll with velocity detection
  useEffect(() => {
  ...
  const handleSmoothScroll = () => {
    ...
    const velocity = timeDiff > 0 ? (scrollDiff / timeDiff) * 1000 : 0;
    ...
    // Parallax effect on background elements - FIXED TYPE ERROR
    const parallaxElements = document.querySelectorAll('.parallax-element');
    parallaxElements.forEach((el) => {
      const element = el as HTMLElement;
      const speedAttr = element.dataset.speed;
      // Convert string to number or use default 0.5
  ...
  useEffect(() => {
    const moveCursor = (e: MouseEvent) => {
      setMousePosition({ x: e.clientX, y: e.clientY });
      if (cursorRef.current) {
        // Add smooth following with delay
  ...
  // Advanced Intersection Observer with smooth thresholds - FIXED TYPE ERRORS
  useEffect(() => {
    const observerOptions = {
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        ...
        // Smooth opacity based on intersection ratio
        target.style.opacity = Math.min(1, ratio * 1.5).toString();
        target.style.transform = `translateY(${20 * (1 - ratio)}px) scale(${0.95 + (ratio * 0.05)})`;
        
        if (ratio > 0.2) {
          target.classList.add('in-view');
          const sectionId = target.getAttribute('data-section');
          if (sectionId && ratio > 0.5) setActiveSection(sectionId);
        } else {
    ...
    // Observe individual cards with stagger effect - FIXED TYPE ERROR
    const cards = document.querySelectorAll('.smooth-card');
    cards.forEach((card, index) => {
      const htmlCard = card as HTMLElement;
      htmlCard.setAttribute('data-index', index.toString());
      htmlCard.style.transition = `all 0.6s cubic-bezier(0.4, 0, 0.2, 1) ${index * 0.1}s`;
  ...
  return (
    <div ref={mainContainerRef} className="min-h-screen bg-background overflow-x-hidden smooth-scroll-container">
      {/* Custom Cursor with smooth follow */}
      <div 
        ref={cursorRef}
        className={`custom-cursor fixed w-8 h-8 pointer-events-none z-50 transition-all duration-300 ease-out ${
      ...
      >
        <div className="w-full h-full rounded-full border-2 border-gold/50 bg-gold/5 backdrop-blur-sm animate-pulse-slow"></div>
      </div>

      <Navigation />

      {/* Smooth Progress Bar with velocity effect */}
      <div className="fixed top-0 left-0 w-full h-1 z-50">
        <div className="relative w-full h-full bg-gold/10">
          <div 
            className="absolute top-0 left-0 h-full bg-gradient-to-r from-gold-dark via-gold to-gold-light transition-all duration-150 ease-out"
            style={{ 
              width: `${scrollProgress}%`,
              filter: `brightness(${1 + scrollVelocity / 1000})`
            }}
          >
            <div className="absolute right-0 top-1/2 transform -translate-y-1/2 w-2 h-2 bg-white rounded-full shadow-lg shadow-gold/50 animate-ping-slow"></div>
          </div>
        </div>
      </div>

      {/* Floating Progress Indicator with smooth rotation */}
      <div className="fixed bottom-8 left-8 z-40 hidden lg:block">
        <div className="relative w-16 h-16 smooth-rotate" style={{ transform: `rotate(${scrollProgress * 3.6}deg)` }}>
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-xs font-bold text-gold smooth-number">
      ...
      {/* Quick Navigation Dots with smooth hover */}
      <div className="fixed right-8 top-1/2 transform -translate-y-1/2 z-40 hidden lg:block">
        <div className="flex flex-col gap-4">
          {[
            { id: 'storage', label: 'Storage' },
            ...
            <div key={item.id} className="relative group">
              <button
                ...
                className={`dot-transition w-3 h-3 rounded-full transition-all duration-500 ease-out ${
                  activeSection === item.id 
                    ? 'bg-gold scale-150 shadow-lg shadow-gold/50' 
                    : 'bg-gold/30 hover:bg-gold/50'
                }`}
              />
              <span className={`absolute right-full mr-3 px-2 py-1 bg-gold text-luxury-black text-xs rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-2 group-hover:translate-x-0 ${
      ...
      <button
        ...
        className={`fixed bottom-8 right-8 z-40 p-3 bg-gold text-luxury-black rounded-full shadow-lg transition-all duration-500 hover:scale-110 hover:shadow-xl ${
          scrollProgress > 20 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20 pointer-events-none'
        }`}
      >
        <ChevronDown className="h-5 w-5 transform rotate-180 smooth-bounce" />
      </button>

      {/* Notification Prompt with smooth entry */}
      {showNotificationPrompt && (
        <div className="fixed bottom-4 right-4 z-50 animate-slide-up">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-4 max-w-sm border border-gray-200 dark:border-gray-700 hover:shadow-2xl transition-all duration-300">
            <div className="flex items-start gap-3">
              <div className="flex-shrink-0">
                <Bell className="h-6 w-6 text-gold animate-ring" />
              </div>
              <div className="flex-1">
                <h3 className="text-sm font-semibold text-gray-900 dark:text-white">
                  Enable Notifications
                ...
                <div className="flex gap-2 mt-3">
                  <Button
                    size="sm"
                    onClick={handleEnableNotifications}
                    className="bg-gold hover:bg-gold-dark text-luxury-black text-xs transition-all duration-300 hover:scale-105"
                  >
                    Enable
                  </Button>
                  <Button
      ...
      {isInitialized && isSupported && (
        <div className="fixed bottom-4 right-4 z-40">
          <div className="flex items-center gap-2 bg-green-500 text-white px-3 py-2 rounded-full shadow-lg animate-pulse-slow">
      ...
      >
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto">
            {/* Bounce In Effect */}
            <div className="inline-flex items-center gap-2 mb-6 px-4 py-2 rounded-full border border-gold/20 bg-white/80 backdrop-blur-sm animate-bounce-in-smooth">
              <Award className="h-4 w-4 text-gold" />
              <span className="text-sm font-medium text-gold-dark">EXPERT GUIDANCE</span>
            </div>
            
            {/* Logo with smooth floating animation */}
            <div className="mb-8 smooth-float">
              <img
                src="https://cdn.builder.io/api/v1/assets/df01e345c2d146ff8c27b0570e833c11/merfume-logo-74e35c?format=webp&width=800"
                alt="Merfume - Luxury Perfumes"
                className="h-24 md:h-32 lg:h-36 w-auto mx-auto drop-shadow-lg"
            ...
            {/* Decoded Effect */}
            <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold text-foreground mb-4 relative">
              <span className="block smooth-reveal">Fragrance Care</span>
              <span 
                ref={decodedRef}
                className="block text-gold bg-gradient-to-r from-gold-dark to-gold bg-clip-text text-transparent pb-2 smooth-reveal-delay"
                onMouseEnter={handleGlitchHover}
              >
                {decodedText || "& MASTERY GUIDE"}
              </span>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <button
                ...
                onMouseLeave={(e) => handleMagneticLeave(e.currentTarget)}
              >
                <Button className="bg-gold hover:bg-gold-dark text-luxury-black px-8 py-4 text-lg rounded-lg w-full group transition-all duration-300 hover:scale-105">
              ...
              <Link to="/store">
                <Button variant="outline" className="border-gold text-gold hover:bg-gold hover:text-luxury-black px-8 py-4 text-lg rounded-lg transition-all duration-300 hover:scale-105">
                  <ShoppingBag className="mr-2 h-5 w-5" />
                  SHOP FRAGRANCES
                </Button>
              </Link>
        ...
        <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 smooth-bounce">
          <div className="w-6 h-10 border-2 border-gold/30 rounded-full flex justify-center">
            <div className="w-1 h-2 bg-gold rounded-full mt-2 animate-scroll-indicator-smooth"></div>
          </div>
        </div>
      </section>

      {/* Quick Storage Tips - Smooth Cards */}
      <section 
        id="storage"
        ref={storageRef}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {storageTips.map((tip, i) => (
              <div 
                key={i}
                className="smooth-card group perspective-1000"
                style={{ transitionDelay: `${i * 0.1}s` }}
              >
                <div className="relative preserve-3d transition-all duration-700 ease-out group-hover:rotate-y-180">
                  {/* Front */}
                  <div className="backface-hidden p-6 bg-gradient-to-br from-cream to-white rounded-lg border border-gold/20 text-center">
                    <div className="w-12 h-12 bg-gold/10 rounded-full flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition-transform duration-300">
                      <div className="text-gold">{tip.icon}</div>
                    </div>
                    <h3 className="font-semibold text-foreground mb-1">{tip.title}</h3>
                    <p className="text-xs text-muted-foreground">{tip.desc}</p>
      ...
      {/* Pulse Points Section with Smooth Reveal */}
      <section 
        id="pulse-points"
        ref={pulsePointsRef}
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            {/* Pulse Points List with Smooth Stagger */}
            <div className="space-y-3">
              {pulsePoints.map((point, index) => (
                <button
                  key={point.id}
                  onClick={() => handlePulsePointClick(point)}
                  className={`w-full text-left p-4 rounded-lg border transition-all duration-500 smooth-card ${
                    showTips === point.label 
                      ? 'border-gold bg-gradient-to-r from-gold/10 to-transparent' 
                      : 'border-border/50 hover:border-gold/30 hover:bg-gold/5'
                  }`}
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <ChevronRight className="h-5 w-5 text-gold/50 group-hover:translate-x-1 transition-transform" />
                  </div>
                </button>
              ))}
            </div>

            {/* Selected Point Details with Smooth Split Reveal */}
            {showTips && (
              <div className="bg-gradient-to-br from-cream to-white rounded-xl p-8 border border-gold/20 smooth-split-reveal">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-2xl font-bold text-foreground">
                    <span className="text-gold">Application Tips</span>
                  </h3>
                  <button
                    onClick={() => setShowTips(null)}
                    className="p-2 hover:bg-gold/10 rounded-full transition-all duration-300 hover:rotate-90"
                  >
                    <X className="h-5 w-5 text-gold" />
                  </button>
                </div>
                ...
                <div className="space-y-6">
                  <div className="p-4 bg-white rounded-lg border border-gold/10 smooth-slide-left">
                    <h4 className="font-semibold text-foreground mb-2 flex items-center gap-2">
                      <Sparkles className="h-4 w-4 text-gold" />
                      How to apply:
                    </h4>
                  ...
                  <div className="p-4 bg-white rounded-lg border border-gold/10 smooth-slide-right">
                    <h4 className="font-semibold text-foreground mb-2 flex items-center gap-2">
                      <Target className="h-4 w-4 text-gold" />
                      Best for:
                    </h4>
                  ...
                  <div className="bg-gradient-to-r from-gold/10 to-transparent p-4 rounded-lg border border-gold/20 smooth-scale-in">
                    <p className="text-sm text-foreground">
                      <span className="font-semibold text-gold">Pro Tip:</span> Apply after moisturizing for better longevity
                    </p>
                  </div>
      ...
      {/* Application Techniques - Smooth Morphing Cards */}
      <section 
        id="techniques"
        ref={techniquesRef}
        className="py-16 bg-card"
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl font-bold text-foreground mb-8 text-center smooth-reveal">
            Application <span className="text-gold">Techniques</span>
          </h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {applicationTechniques.map((tech, i) => (
              <div 
                key={i}
                className="group relative h-48 smooth-card"
                style={{ transitionDelay: `${i * 0.1}s` }}
              >
                <div className="absolute inset-0 bg-gradient-to-br from-white to-cream rounded-xl border border-gold/20 p-6 text-center transition-all duration-500 ease-out group-hover:scale-105 group-hover:shadow-xl">
                  <div className="w-12 h-12 bg-gold/10 rounded-full flex items-center justify-center mx-auto mb-3 group-hover:bg-gold/20 transition-all duration-300">
                    <div className="text-gold group-hover:scale-110 transition-transform duration-300">{tech.icon}</div>
                  </div>
                  <h3 className="font-semibold text-foreground mb-1">{tech.title}</h3>
                  <p className="text-xs text-muted-foreground">{tech.desc}</p>
                </div>
      ...
      {/* Timeline Section with Smooth Parallax */}
      <section 
        id="timeline"
        ref={timelineRef}
        className="py-20 bg-background relative overflow-hidden"
      >
        <div className="parallax-element absolute inset-0 bg-gradient-to-b from-gold/5 via-transparent to-gold/5" data-speed="0.2"></div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-12 smooth-sticky">
            <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4">
              Fragrance <span className="text-gold">Evolution</span> Timeline
            </h2>
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {timelinePhases.map((phase, i) => (
              <div 
                key={i} 
                className="group relative smooth-card hover:-translate-y-2 transition-all duration-500"
                style={{ 
                  transitionDelay: `${i * 0.15}s` 
                }}
              >
                {/* Smooth glowing effect */}
                <div className="absolute inset-0 bg-gold/5 rounded-xl blur-xl group-hover:blur-2xl transition-all duration-700"></div>
                
                <div className="relative bg-gradient-to-br from-white to-cream border border-gold/20 rounded-xl p-6">
                  <div className="mb-4">
                    <div className="text-sm text-gold-dark font-semibold mb-2">{phase.time}</div>
                    <div className="h-1.5 w-full bg-gold/10 rounded-full overflow-hidden">
                      <div className="h-full w-full bg-gradient-to-r from-gold-dark to-gold rounded smooth-wave" />
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {phase.notes.map((note, idx) => (
                      <span key={idx} className="px-2 py-1 bg-white border border-gold/20 rounded-full text-xs text-foreground hover:bg-gold/5 transition-all duration-300 hover:scale-105">
                        {note}
                      </span>
                    ))}
                  </div>
      ...
      {/* Longevity Tips - Smooth Binary Effect */}
      <section 
        id="longevity"
        ref={longevityRef}
        className="py-16 bg-card"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {longevityTips.map((tip, i) => (
              <div 
                key={i}
                className="smooth-card bg-gradient-to-br from-white to-cream p-4 rounded-lg border border-gold/20 binary-decoded relative hover:shadow-xl transition-all duration-500 hover:-translate-y-1"
                data-binary={tip}
                style={{ transitionDelay: `${i * 0.1}s` }}
              >
                <div className="flex items-start gap-3">
                  <div className="relative">
                    <Check className="h-5 w-5 text-gold mt-0.5 flex-shrink-0" />
                    <div className="absolute inset-0 bg-gold/20 rounded-full blur-md animate-ping-slow"></div>
                  </div>
                  <span className="text-sm text-foreground">{tip}</span>
                </div>
              </div>
      ...
      {/* Seasonal Recommendations - Smooth Wavy Cards */}
      <section 
        id="seasonal"
        ref={seasonalRef}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {seasonalRecs.map((season, i) => (
              <div 
                key={i}
                className="smooth-card group relative overflow-hidden"
                style={{ transitionDelay: `${i * 0.1}s` }}
              >
                {/* Smooth rotating gradient */}
                <div className="absolute inset-0 bg-gradient-to-br from-gold/10 via-transparent to-gold/10 opacity-0 group-hover:opacity-100 transition-opacity duration-700"></div>
                
                <div className="relative smooth-wave-card p-6 bg-gradient-to-br from-cream to-white rounded-xl border border-gold/20 hover:shadow-xl transition-all duration-500">
                  <h3 className="font-bold text-xl text-gold mb-3">{season.season}</h3>
                  <div className="space-y-2 mb-3">
                    {season.scents.map((scent, idx) => (
                      <span key={idx} className="block text-sm text-foreground">{scent}</span>
      ...
      {/* Care Tips Grid with Smooth 3D Tilt */}
      <section 
        id="care-tips"
        ref={careTipsRef}
            ...
            {careTips.map((tip, i) => (
              ...
              >
                <div className="relative preserve-3d transition-all duration-700 ease-out group-hover:rotate-y-180">
                  {/* Front */}
                  <div className="backface-hidden bg-gradient-to-br from-white to-cream border border-gold/20 rounded-xl p-6">
                    <div className="w-14 h-14 rounded-full bg-gold/10 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300">
                      <div className="text-gold">{tip.icon}</div>
                    </div>
                    <h3 className="text-lg font-bold text-foreground mb-3">{tip.title}</h3>
                    <ul className="space-y-2">
                  ...
                  <div className="absolute inset-0 backface-hidden rotate-y-180 bg-gold text-luxury-black rounded-xl p-6 flex items-center justify-center">
                    <div className="text-center">
                      <Sparkles className="h-8 w-8 mx-auto mb-3 animate-pulse-slow" />
                      <p className="text-sm font-semibold">Expert Approved</p>
                    </div>
                  </div>
                </div>
      ...
      {/* Ingredients Guide with Smooth Magnetic Pull */}
      <section 
        id="ingredients"
        ref={ingredientsRef}
        className="py-20 bg-background relative"
      >
        ...
        <div className="absolute inset-0 overflow-hidden">
          {[...Array(20)].map((_, i) => (
            <div
              key={i}
              className="absolute w-1 h-1 bg-gold/20 rounded-full smooth-float-particle"
              style={{
                left: `${Math.random() * 100}%`,
                top: `${Math.random() * 100}%`,
                animation: `smooth-float-particle ${5 + Math.random() * 10}s linear infinite`,
                animationDelay: `${Math.random() * 5}s`
              }}
            />
          ))}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {ingredients.map((ingredient, i) => (
              <div 
                key={i} 
                className="smooth-card bg-gradient-to-br from-white to-cream border border-gold/20 rounded-xl p-4 text-center magnetic-card hover:shadow-xl transition-all duration-300"
                onMouseMove={(e) => handleMagneticMove(e, e.currentTarget)}
                onMouseLeave={(e) => handleMagneticLeave(e.currentTarget)}
                style={{ transitionDelay: `${i * 0.1}s` }}
              >
      ...
      {/* Mood-Based Selection with Smooth Glowing Cards */}
      <section 
        id="mood"
        ref={moodRef}
        className="py-20 bg-card"
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {moodOptions.map((item, index) => (
              <button
                key={item.mood}
                onClick={() => handleMoodSelect(item.mood)}
                className={`smooth-card group relative ${
                  selectedMood === item.mood ? 'selected-glow' : ''
                }`}
                style={{ transitionDelay: `${index * 0.1}s` }}
              >
                {/* Smooth glowing effect */}
                <div className={`absolute inset-0 bg-gold/20 rounded-xl blur-xl transition-opacity duration-700 ${
                  selectedMood === item.mood ? 'opacity-100' : 'opacity-0 group-hover:opacity-50'
                }`}></div>
                
                <div className={`relative bg-gradient-to-br from-white to-cream border-2 rounded-xl p-6 text-center transition-all duration-500 ${
                  selectedMood === item.mood 
                    ? 'border-gold shadow-lg' 
                    : 'border-gold/20 hover:border-gold/50'
                }`}>
                  <div className="w-14 h-14 bg-gold/10 rounded-full flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition-transform duration-300">
                    <div className="text-gold">{item.icon}</div>
                  </div>
                  <h3 className="font-bold text-foreground text-sm mb-1">{item.mood}</h3>
                  <p className="text-xs text-muted-foreground mb-2">{item.desc}</p>
          ...
          {/* Mood Recommendation with Smooth Split Reveal */}
          {selectedMood && (
            <div className="bg-gradient-to-br from-cream to-white border border-gold/20 rounded-xl p-8 smooth-split-reveal">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-2xl font-bold text-foreground">
                  Recommended for: <span className="text-gold">{selectedMood}</span>
                </h3>
              ...
              <div className="grid md:grid-cols-3 gap-6">
                <div className="space-y-3 smooth-slide-left">
                  <h4 className="font-semibold text-foreground flex items-center gap-2">
                    <Sparkles className="h-4 w-4 text-gold" />
                    Scent Families
                  </h4>
                  <div className="space-y-2">
                    {moodOptions.find(m => m.mood === selectedMood)?.notes.map((note, i) => (
                      <div key={i} className="bg-white p-3 rounded-lg border border-gold/20 text-sm text-foreground hover:shadow-md transition-all duration-300 hover:scale-105">
                        {note}
                      </div>
                    ))}
                  </div>
                ...
                <div className="space-y-3 smooth-slide-bottom" style={{ animationDelay: '0.2s' }}>
                  <h4 className="font-semibold text-foreground flex items-center gap-2">
                    <Target className="h-4 w-4 text-gold" />
                    Perfect For
                  </h4>
                ...
                <div className="space-y-3 smooth-slide-right" style={{ animationDelay: '0.3s' }}>
                  <h4 className="font-semibold text-foreground flex items-center gap-2">
                    <BookOpen className="h-4 w-4 text-gold" />
                    Application Guide
                  </h4>
                  <div className="space-y-2">
                    <div className="flex justify-between bg-white p-3 rounded-lg border border-gold/20 hover:shadow-md transition-all duration-300 hover:scale-105">
                      <span className="text-sm text-muted-foreground">Strength</span>
                      <span className="text-gold font-bold">
                        {moodOptions.find(m => m.mood === selectedMood)?.strength}
                      </span>
      ...
      {/* Stats Section with Smooth Counter */}
      <section 
        id="stats"
        ref={statsRef}
        className="py-16 bg-gradient-to-r from-gold/10 via-accent/20 to-gold/10"
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              <div 
                key={i} 
                className="smooth-card group bg-white/80 backdrop-blur-sm rounded-xl p-6 text-center border border-gold/20 hover:shadow-xl transition-all duration-500 hover:-translate-y-2"
                style={{ transitionDelay: `${i * 0.1}s` }}
              >
                <div className="w-14 h-14 rounded-full bg-gold/10 flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition-transform duration-300">
                  <div className="text-gold">{stat.icon}</div>
                </div>
                <div className="text-3xl font-bold text-gold mb-1 smooth-counter">{stat.value}</div>
                <div className="text-sm text-muted-foreground">{stat.label}</div>
              </div>
            ))}
          </div>
      ...
      {/* CTA Section with Smooth Particle Background */}
      <section 
        id="cta"
        ref={ctaRef}
        className="py-20 bg-background relative overflow-hidden"
      >
        ...
        <div className="absolute inset-0">
          {[...Array(50)].map((_, i) => (
            <div
              key={i}
              className="absolute w-1 h-1 bg-gold/10 rounded-full smooth-float-particle"
              style={{
                left: `${Math.random() * 100}%`,
                top: `${Math.random() * 100}%`,
                animation: `smooth-float-particle ${10 + Math.random() * 20}s linear infinite`,
                animationDelay: `${Math.random() * 10}s`
              }}
            />
          ))}
        ...
        <div className="max-w-4xl mx-auto text-center px-4 sm:px-6 lg:px-8 relative z-10">
          <h2 
            className="text-3xl md:text-4xl font-bold text-foreground mb-4 relative glitch-trigger smooth-reveal"
            onMouseEnter={handleGlitchHover}
          >
            Ready to Elevate Your <span className="text-gold">Fragrance Journey?</span>
          </h2>
          
          <p className="text-lg text-muted-foreground mb-8 max-w-2xl mx-auto smooth-reveal-delay">
            Apply these expert techniques to transform your fragrance experience
          </p>
          
          <div className="flex flex-col sm:flex-row gap-4 justify-center smooth-scale-in">
            <Link to="/store">
              <Button className="bg-gold hover:bg-gold-dark text-luxury-black px-8 py-4 text-lg rounded-lg font-semibold shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 group">
            ...
            <Link to="/contact">
              <Button variant="outline" className="border-gold text-gold hover:bg-gold hover:text-luxury-black px-8 py-4 text-lg rounded-lg transition-all duration-300 hover:scale-105 group">
      ...
      <style>{`
        ...
        .smooth-reveal-delay {
          animation: smoothReveal 1s cubic-bezier(0.4, 0, 0.2, 1) 0.3s forwards;
          opacity: 0;
        }
        ...
        /* Smooth Split Reveal */
        .smooth-split-reveal {
          animation: smoothSplitReveal 0.8s cubic-bezier(0.4, 0, 0.2, 1) forwards;
        }

        @keyframes smoothSplitReveal {
          0% {
        ...
        .smooth-slide-left {
          animation: smoothSlideLeft 0.6s cubic-bezier(0.4, 0, 0.2, 1) forwards;
        }

        .smooth-slide-right {
          animation: smoothSlideRight 0.6s cubic-bezier(0.4, 0, 0.2, 1) forwards;
        }

        .smooth-slide-bottom {
          animation: smoothSlideBottom 0.6s cubic-bezier(0.4, 0, 0.2, 1) forwards;
        }
        ...
        /* Smooth Wave Card */
        .smooth-wave-card {
          animation: smoothWaveCard 3s ease-in-out infinite;
        }

        @keyframes smoothWaveCard {
          0%, 100% {
            transform: translateY(0);
          }
          25% {
        ...
        /* Smooth Float Particle */
        .smooth-float-particle {
          animation: smoothFloatParticle linear infinite;
        }

        @keyframes smoothFloatParticle {
          0% {
        ...
        /* Smooth Scroll Indicator */
        .animate-scroll-indicator-smooth {
          animation: smoothScrollIndicator 2s ease-in-out infinite;
        }
        ...
        .animate-ping-slow {
          animation: smoothPing 2s cubic-bezier(0, 0, 0.2, 1) infinite;
        }
        ...
        .animate-bounce-in-smooth {
          animation: bounceInSmooth 0.8s cubic-bezier(0.4, 0, 0.2, 1) forwards;
        }
