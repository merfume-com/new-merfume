...
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { ShoppingCart, Heart, Star, Eye, Check, Zap, AlertCircle } from "lucide-react";
import { toast } from "sonner";
import axios from "axios";
import { motion } from "framer-motion";
import {
...
export default function ProductCard({
  ...
  const handleAddToCart = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        // If error is about stock, show more specific message
  ...
  if (variant === "grid") {
    return (
      <motion.div
        onClick={handleProductClick}
        className={`cursor-pointer group relative ${className} ${isOutOfStock ? 'opacity-80' : ''}`}
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      >
        <Card className={`h-full border ${isOutOfStock ? 'border-red-200 dark:border-red-900/30' : 'border-border/40 hover:border-gold/30'} bg-gradient-to-b from-background to-background/95 overflow-hidden transition-all duration-300 rounded-xl ${!isOutOfStock && 'hover:shadow-lg'}`}>
          {/* Image Section */}
          <div className="relative aspect-square overflow-hidden">
            <motion.div
            ...
            {originalPrice && discount > 0 && !isOutOfStock && (
              <div className="absolute top-3 left-3 bg-gradient-to-r from-red-500 to-red-600 text-white text-xs font-bold px-2 py-1 rounded-md shadow-md z-10">
                -{discount}%
              </div>
            )}
              ...
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div className="absolute top-3 left-3 bg-gradient-to-r from-yellow-500 to-yellow-600 text-white text-xs font-bold px-2 py-1 rounded-md shadow-md cursor-help z-10">
                      Only {stockQuantity} left
                    </div>
                  </TooltipTrigger>
                  <TooltipContent>
                    <p>Hurry! Only {stockQuantity} items in stock</p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            )}
            ...
            {isOutOfStock && (
              <div className="absolute top-3 left-3 bg-gradient-to-r from-red-600 to-red-700 text-white text-xs font-bold px-3 py-1.5 rounded-md shadow-md z-10 flex items-center gap-1">
            ...
            {isOutOfStock && (
              <div className="absolute inset-0 bg-black/50 backdrop-blur-[2px] flex items-center justify-center z-20">
                <div className="bg-red-600 text-white px-4 py-2 rounded-lg font-bold transform -rotate-12 shadow-lg">
                  OUT OF STOCK
                </div>
              </div>
            )}
            ...
            <div className="absolute top-3 right-3 flex flex-col gap-2 z-10">
              ...
              {productBackImageUrl && !isOutOfStock && (
                <button
                  onClick={handleEyeClick}
                  className="bg-white/90 backdrop-blur-sm p-2 rounded-full hover:bg-white transition-all duration-200 shadow-sm hover:scale-110 active:scale-95"
                >
          ...
          <CardContent className="p-4 flex flex-col h-[240px]">
            {/* Brand and Rating */}
            <div className="flex items-center justify-between mb-2">
              <span className={`text-sm font-medium ${isOutOfStock ? 'text-gray-400' : 'text-gold'} bg-gold/10 px-2 py-1 rounded-full`}>
                {brand}
              </span>
              <div className="flex items-center gap-1">
                <div className="flex">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      ...
                      className={`h-3 w-3 ${
                        i < Math.floor(rating)
                          ? isOutOfStock ? "text-gray-400" : "text-gold fill-current"
                          : "text-muted-foreground/30"
                      }`}
                    />
                  ))}
                </div>
                <span className={`text-xs font-semibold ml-1 ${isOutOfStock ? 'text-gray-400' : ''}`}>{rating.toFixed(1)}</span>
                <span className="text-xs text-muted-foreground">({reviewCount})</span>
              </div>
            </div>

            {/* Product Name */}
            <h3 className={`text-base font-bold ${isOutOfStock ? 'text-gray-400' : 'text-foreground'} mb-2 line-clamp-2 h-12 ${!isOutOfStock && 'hover:text-gold'} transition-colors`}>
              {productName}
            </h3>
            ...
            <div className="mt-auto pt-3 border-t border-border/20">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="space-y-1">
                  <div className="flex items-baseline gap-2 flex-wrap">
                    <span className={`text-xl font-bold ${isOutOfStock ? 'text-gray-400' : 'text-gold'}`}>
                      ₹{productPrice.toLocaleString()}
                    </span>
                    {originalPrice && (
                      <>
                ...
                <Button
                  ...
                  className={`${
                    isOutOfStock
                      ...
                      : 'bg-gradient-to-r from-gold to-gold/90 hover:from-gold/90 hover:to-gold text-black'
                  } font-semibold rounded-lg px-4 py-2 w-full sm:w-auto min-w-[110px] z-10`}
                  onClick={handleAddToCart}
                  disabled={isAdding || added || isOutOfStock}
                >
                  {isAdding ? (
                    <div className="flex items-center gap-1.5 justify-center">
                      <div className="h-3 w-3 border-2 border-black border-t-transparent rounded-full animate-spin"></div>
  ...
  return (
    <motion.div
      onClick={handleProductClick}
      className={`cursor-pointer group ${className} ${isOutOfStock ? 'opacity-80' : ''}`}
      initial={{ opacity: 0, x: -20 }}
      animate={{ opacity: 1, x: 0 }}
      transition={{ duration: 0.3 }}
    >
      <Card className={`border ${isOutOfStock ? 'border-red-200 dark:border-red-900/30' : 'border-border/40 hover:border-gold/30'} bg-gradient-to-r from-background to-background/95 overflow-hidden transition-all duration-300 rounded-xl ${!isOutOfStock && 'hover:shadow-lg'}`}>
        {/* Mobile Layout */}
        <div className="flex flex-col md:flex-row">
          {/* Image Section */}
          <div className="md:w-48 lg:w-56 relative h-48 md:h-auto md:min-h-[200px]">
            <div className="relative h-full w-full">
              ...
              {/* Stock Badge */}
              {!isOutOfStock && isLowStock && stockQuantity > 0 && (
                <div className="absolute top-3 left-3 bg-gradient-to-r from-yellow-500 to-yellow-600 text-white text-xs font-bold px-2 py-1 rounded-md shadow-md z-10">
                  Only {stockQuantity} left
                </div>
              )}
          ...
          <CardContent className="flex-1 p-4 md:p-6">
            <div className="h-full flex flex-col">
              ...
              <div className="hidden md:block mb-3">
                <div className="flex items-center justify-between mb-2">
                  <span className={`text-sm font-medium ${isOutOfStock ? 'text-gray-400' : 'text-gold'} bg-gold/10 px-3 py-1 rounded-full`}>
                    {brand}
                  </span>
                  <div className="flex items-center gap-1">
                    <div className="flex">
                    ...
                    <span className={`text-sm font-semibold ml-1 ${isOutOfStock ? 'text-gray-400' : ''}`}>{rating.toFixed(1)}</span>
                    <span className="text-sm text-muted-foreground">({reviewCount})</span>
                  </div>
                </div>
              </div>
              ...
              {/* Product Name */}
              <h3 className={`text-lg md:text-xl font-bold ${isOutOfStock ? 'text-gray-400' : 'text-foreground'} mb-2 line-clamp-2 ${!isOutOfStock && 'hover:text-gold'} transition-colors`}>
                {productName}
              </h3>
                    ...
                    <span
                      ...
                      className={`text-xs ${
                        isOutOfStock 
                          ? 'bg-gray-200 dark:bg-gray-800 text-gray-500' 
                          : 'bg-gradient-to-r from-accent/80 to-accent/60 text-accent-foreground'
                      } px-2 md:px-3 py-1 md:py-1.5 rounded-full font-medium`}
                    >
                      {note}
                    </span>
                  ))}
                  ...
                  <Button
                    className={`w-full ${
                      isOutOfStock
                        ? 'bg-gray-400 cursor-not-allowed'
                        : 'bg-gradient-to-r from-gold to-gold/90 hover:from-gold/90 hover:to-gold text-black'
                    } font-semibold rounded-lg py-3 z-10`}
                    onClick={handleAddToCart}
                    disabled={isAdding || added || isOutOfStock}
                  >
                    {isAdding ? (
              ...
              <div className="hidden md:block mt-auto pt-4 border-t border-border/20">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="space-y-2">
                    <div className="flex items-baseline gap-3">
                      <span className={`text-2xl md:text-3xl font-bold ${isOutOfStock ? 'text-gray-400' : 'text-gold'}`}>
                        ₹{productPrice.toLocaleString()}
                      </span>
                      {originalPrice && (
                        <>
                    ...
                    <div className="flex items-center gap-2">
                      <Zap className={`h-4 w-4 ${isOutOfStock ? 'text-gray-400' : 'text-green-500'}`} />
                      <p className="text-sm text-muted-foreground">
                        Free shipping on orders over ₹500
                      </p>
                    </div>
                    ...
                    <Button
                      ...
                      className={`${
                        isOutOfStock
                          ? 'bg-gray-400 cursor-not-allowed'
                          : 'bg-gradient-to-r from-gold to-gold/90 hover:from-gold/90 hover:to-gold text-black'
                      } font-semibold rounded-lg px-6 py-3 min-w-[140px] z-10`}
                      onClick={handleAddToCart}
                      disabled={isAdding || added || isOutOfStock}
                    >
                      {isAdding ? (
