...
api.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    if (!error.response && !originalRequest?._retry && originalRequest?.method?.toLowerCase() === 'get') {
      originalRequest._retry = true;
      console.log("Retrying request due to network error...");
...
export default function AdminDashboard() {
    ...
    try {
      ...
      if (Array.isArray(response.data)) {
        const productsWithStock = response.data.map((product: any) => ({
          ...
          isOutOfStock: product.isOutOfStock !== undefined ? product.isOutOfStock : (product.stockQuantity <= 0)
        }));
        
        setProducts(productsWithStock);
        
        // Update stats
        const lowStock = productsWithStock.filter((p: Product) => p.isLowStock && !p.isDeleted).length;
        const outOfStock = productsWithStock.filter((p: Product) => p.isOutOfStock && !p.isDeleted).length;
        
        setStats(prev => ({
          ...prev,
          activeProducts: productsWithStock.length,
  ...
  const fetchDeletedProducts = useCallback(async () => {
    ...
    try {
      ...
      if (Array.isArray(response.data)) {
        const deletedWithStock = response.data.map((product: any) => ({
          ...product,
          stockQuantity: product.stockQuantity || 0,
          lowStockThreshold: product.lowStockThreshold || 5,
          trackInventory: product.trackInventory !== false,
        ...
        setDeletedProducts(deletedWithStock);
        
        // Update stats
        setStats(prev => ({
          ...prev,
          deletedProducts: deletedWithStock.length
        }));
      } else {
        console.error("Invalid response format: expected array");
        setDeletedProducts([]);
  ...
  const handleUpdateProduct = async (productId: number) => {
    ...
    try {
        ...
        const productData = {
            ...
            trackInventory: selectedProduct.trackInventory !== false,
            allowBackorders: selectedProduct.allowBackorders || false
        };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* Stock Alert Cards - Only show on products tab */}
      {activeTab === "products" && !showDeletedProducts && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {stats.lowStockProducts > 0 && (
            <Alert className="bg-yellow-50 border-yellow-200">
              <AlertTriangle className="h-4 w-4 text-yellow-600" />
              <AlertDescription className="text-yellow-800">
                <span className="font-semibold">{stats.lowStockProducts}</span> product{stats.lowStockProducts !== 1 ? 's' : ''} {stats.lowStockProducts !== 1 ? 'are' : 'is'} running low on stock.
                <Button variant="link" className="text-yellow-800 font-semibold px-2" onClick={() => setSearchQuery("")}>
                  View products
                </Button>
              </AlertDescription>
            </Alert>
          ...
          {stats.outOfStockProducts > 0 && (
            <Alert className="bg-red-50 border-red-200">
              ...
              <AlertDescription className="text-red-800">
                <span className="font-semibold">{stats.outOfStockProducts}</span> product{stats.outOfStockProducts !== 1 ? 's' : ''} {stats.outOfStockProducts !== 1 ? 'are' : 'is'} out of stock.
                <Button variant="link" className="text-red-800 font-semibold px-2" onClick={() => setSearchQuery("")}>
                  View products
                </Button>
              </AlertDescription>
            </Alert>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="space-y-4 border-t pt-4">
                          ...
                          <div>
                            ...
                            <Input
                              type="number"
                              value={newProduct.stockQuantity || 100}
                          ...
                          <div className="flex items-center space-x-2">
                            <Switch
                              checked={newProduct.allowBackorders}
                              onCheckedChange={(checked) => setNewProduct({...newProduct, allowBackorders: checked})}
                              id="allow-backorders"
                            />
                  ...
                  <TableBody>
                    {filteredProducts.map((product) => (
                      <TableRow key={product.productId} className="hover:bg-gray-50">
                        ...
                        <TableCell>
                          {!showDeletedProducts && getStockStatusBadge(product)}
                          {showDeletedProducts && (
                            <span className="text-sm text-muted-foreground">
                              ...
                              <>
                                <TooltipProvider>
                                  <Tooltip>
                                    ...
                                    <TooltipContent>Restore product</TooltipContent>
                                  </Tooltip>
                                </TooltipProvider>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center space-x-2">
                      <Switch
                        checked={selectedProduct.trackInventory}
                        onCheckedChange={(checked) => setSelectedProduct({...selectedProduct, trackInventory: checked})}
                        id="edit-track-inventory"
                      />
                      <Label htmlFor="edit-track-inventory">Track Inventory</Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Switch
                        checked={selectedProduct.allowBackorders}
                        onCheckedChange={(checked) => setSelectedProduct({...selectedProduct, allowBackorders: checked})}
                        id="edit-allow-backorders"
                      />
                      <Label htmlFor="edit-allow-backorders">Allow Backorders</Label>
                    </div>
                  </div>
